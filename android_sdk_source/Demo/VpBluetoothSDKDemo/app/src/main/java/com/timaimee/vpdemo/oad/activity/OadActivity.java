...
public class OadActivity extends Activity implements View.OnClickListener {
    ...
    private void initDeviceStateView() {
        ...
        sb.append("\n");
        sb.append("Equipment Number:" + oadSetting.getDeviceNumber());
        sb.append("\n");
        sb.append("Official release number:" + oadSetting.getDeviceVersion());
        sb.append("\n");
    ...
    private void checkVersionAndFile() {
        ...
        Logger.t(TAG).i("Before Upgrade: Version Verification-> File Verification-> Find Target Device");
        VPOperateManager.getMangerInstance(mContext).checkVersionAndFile(oadSetting, new OnUpdateCheckListener() {
            ...
            public void onNetVersionInfo(int deviceNumber, String deviceVersion, String des) {
                Logger.t(TAG).i("Server version information, device number =" + deviceNumber + ", latest version =" + deviceVersion + ", upgrade description =" + des);
            }
            ...
            public void onCheckFail(int endState) {
                switch (endState) {
                    ...
                    case OadErrorState.UNCONNECT_SERVER:
                        Logger.t(TAG).i("Server cannot connect");
                        break;
                    case OadErrorState.SERVER_NOT_HAVE_NEW:
                        Logger.t(TAG).i("Server does not have this version");
                        break;
            ...
            public void onCheckSuccess(String oadFileName) {
                Logger.t(TAG).i("Verifications are correct, documents are correct");
            ...
            public void findOadDevice(String oadAddress) {
                Logger.t(TAG).i("Device found in OAD mode");
    ...
    private void startOad() {
        Logger.t(TAG).i("Run upgrade procedure, try up to 5 times");
        showProgressBar();
    ...
    private void oadSuccess() {
        ...
        disMissProgressBar();
        Toast.makeText(mContext, "Upgrade successful", Toast.LENGTH_SHORT).show();
        Logger.t(TAG).e("Upgrade successful");
        this.finish();
    }
    ...
    private void oadFail() {
        ...
        if (failCount < MAX_ALLOW_FAIL_COUNT) {
            Logger.t(TAG).e("Try again =" + failCount);
            showProgressBar();
            startOad();
        } else {
            showOadFailDialog();
    ...
    private void showOadFailDialog() {
        isFindOadDevice = false;
        String mStringContent = "Upgrade failed, device name will change to DfuLang";
        String mStringTitle = "Tips";
        String mStringOk = "I see";
